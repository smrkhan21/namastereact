import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faStar, faCaretDown, faCaretUp } from '@fortawesome/free-solid-svg-icons'
import { useEffect, useState } from "react";
import { RES_MENU } from "../../utils/config";
import Shimmer from "./Shimmer";
import { useParams } from "react-router-dom";

const RestaurantMenu = () => {
    const [caretState, setCaretState] = useState(faCaretUp);
    const [resMenu, setResMenu] = useState(null)
    const {resId} = useParams()
    
    useEffect(() => {
        fetchMenu()
    }, [])
    
    const fetchMenu = async () => {
        const data = await fetch(RES_MENU + resId)
        const json = await data.json();
        console.log(json)
        setResMenu(json);
    }

    if(resMenu === null) {
        return (
            <Shimmer/>
        )
    }

    const { name, cuisines, areaName, avgRatingString, totalRatingsString, costForTwo, sla} = resMenu?.data?.cards[0]?.card?.card?.info;


    return (
        <div className="menu">
            <div className="restaurant-detail">
                <div className="left">
                    <p className="rest-name">{name}</p>
                    <p className="other">{cuisines.join(", ")}</p>
                    <p className="distance">{areaName}, {sla?.lastMileTravelString}</p>
                </div>
                <div className="right rating">
                    <p className="star"><FontAwesomeIcon icon={faStar} /> {avgRatingString}</p>
                    <p className="ratings">{totalRatingsString}</p>
                </div>
            </div>
            <div className="offers">
                <p className="eta">
                    <svg className="svg RestaurantTimeCost_icon__8UdT4" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" fill="none"><circle r="8.35" transform="matrix(-1 0 0 1 9 9)" stroke="#3E4152" strokeWidth="1.3"></circle><path d="M3 15.2569C4.58666 16.9484 6.81075 18 9.273 18C14.0928 18 18 13.9706 18 9C18 4.02944 14.0928 0 9.273 0C9.273 2.25 9.273 9 9.273 9C6.36399 12 5.63674 12.75 3 15.2569Z" fill="#3E4152"></path></svg>
                    <span>{sla.deliveryTime + " Min"}</span>
                </p>
                <p className="cft">
                    <svg className="svg RestaurantTimeCost_icon__8UdT4" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" fill="none"><circle cx="9" cy="9" r="8.25" stroke="#3E4152" strokeWidth="1.5"></circle><path d="M12.8748 4.495H5.6748V6.04H7.9698C8.7948 6.04 9.4248 6.43 9.6198 7.12H5.6748V8.125H9.6048C9.3798 8.8 8.7648 9.22 7.9698 9.22H5.6748V10.765H7.3098L9.5298 14.5H11.5548L9.1098 10.57C10.2048 10.39 11.2698 9.58 11.4498 8.125H12.8748V7.12H11.4348C11.3148 6.475 10.9698 5.905 10.4298 5.5H12.8748V4.495Z" fill="#3E4152"></path></svg>
                    <span>{"Rs. " + costForTwo/100 + " for Two"}</span>
                </p>
            </div>
            <div className="filter">Veg Only Filter</div>
            <div className="menu-types">
                <div className="type">
                    <div className="type-header">
                        <h4>Leon Burgers And Burger Combos (23)</h4>
                        <FontAwesomeIcon onClick={() => setCaretState(caretState == faCaretUp ? faCaretDown : faCaretUp )} icon={caretState}/>
                    </div>
                    <div className={"restaurant-items-content "+ (caretState == faCaretUp ? "active" : "")}>
                        <div className="rest-content">
                            <div className="item">
                                <p className="itemName">Paneer Butter Masala</p>
                                <p className="itemPrice">
                                    <span className="original-price">Rs.250 </span> 
                                    <span className="discounted-price">Rs.130</span>
                                </p>
                                <p className="description">
                                    Serves 1 | A popular paneer delight, creamy and indulgent dish cooked in a onion tomato gravy.(serves1)
                                </p>
                            </div>
                            <div className="item-image">
                                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/jodq0fyqrf056qpz7ope" width={100} height={100}/>
                            </div>
                        </div>
                        <div className="rest-content">
                            <div className="item">
                                <p className="itemName">Paneer Butter Masala</p>
                                <p className="itemPrice">
                                    <span className="original-price">Rs.250 </span> 
                                    <span className="discounted-price">Rs.130</span>
                                </p>
                                <p className="description">
                                    Serves 1 | A popular paneer delight, creamy and indulgent dish cooked in a onion tomato gravy.(serves1)
                                </p>
                            </div>
                            <div className="item-image">
                                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/jodq0fyqrf056qpz7ope" width={100} height={100}/>
                            </div>
                        </div>
                        <div className="rest-content">
                            <div className="item">
                                <p className="itemName">Paneer Butter Masala</p>
                                <p className="itemPrice">
                                    <span className="original-price">Rs.250 </span> 
                                    <span className="discounted-price">Rs.130</span>
                                </p>
                                <p className="description">
                                    Serves 1 | A popular paneer delight, creamy and indulgent dish cooked in a onion tomato gravy.(serves1)
                                </p>
                            </div>
                            <div className="item-image">
                                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/jodq0fyqrf056qpz7ope" width={100} height={100}/>
                            </div>
                        </div>
                        <div className="rest-content">
                            <div className="item">
                                <p className="itemName">Paneer Butter Masala</p>
                                <p className="itemPrice">
                                    <span className="original-price">Rs.250 </span> 
                                    <span className="discounted-price">Rs.130</span>
                                </p>
                                <p className="description">
                                    Serves 1 | A popular paneer delight, creamy and indulgent dish cooked in a onion tomato gravy.(serves1)
                                </p>
                            </div>
                            <div className="item-image">
                                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/jodq0fyqrf056qpz7ope" width={100} height={100}/>
                            </div>
                        </div>
                        <div className="rest-content">
                            <div className="item">
                                <p className="itemName">Paneer Butter Masala</p>
                                <p className="itemPrice">
                                    <span className="original-price">Rs.250 </span> 
                                    <span className="discounted-price">Rs.130</span>
                                </p>
                                <p className="description">
                                    Serves 1 | A popular paneer delight, creamy and indulgent dish cooked in a onion tomato gravy.(serves1)
                                </p>
                            </div>
                            <div className="item-image">
                                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/jodq0fyqrf056qpz7ope" width={100} height={100}/>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default RestaurantMenu;